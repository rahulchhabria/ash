...
| `PIONEER_API_KEY` | Pioneer API key (for `provider = "pioneer"`) |
...
| `[openai]`, `[anthropic]`, `[pioneer]` | Provider API key config |
...
| `[parallel_search]`, `[exa_search]`, `[google_places]`, `[sentry]`, `[server]` | Optional subsystem config |
