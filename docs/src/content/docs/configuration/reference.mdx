...
| `network_mode` | `none` \| `bridge` | `none` |
| `dns_servers` | list[string] | `[]` |
| `http_proxy` | string \| null | `null` |
| `workspace_access` | `none` \| `ro` \| `rw` | `rw` |
| `github_auth_access` | `none` \| `ro` | `none` |
| `sessions_access` | `none` \| `ro` | `none` |
| `chats_access` | `none` \| `ro` | `none` |
| `logs_access` | `none` \| `ro` | `none` |
...
| `retention_sweep_seconds` | int | `60` |
...
| `browser.kernel.headless` | bool | `true` |
| `browser.kernel.stealth` | bool | `true` |
| `browser.kernel.session_timeout_seconds` | int | `300` |
...
Kernel sessions default to the lower-cost headless tier and a five-minute
server-side timeout. Raise these values only for workflows that need them.
...
Set `dry_run = true` to test the Telegram approval flow without Vapi credentials or
network call creation; the tool validates the request and returns a dry-run summary.
