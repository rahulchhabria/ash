...
Hosted OpenAI search is the default when the active model supports it. Configure
optional provider fallbacks or local-business lookup as needed:
...
If tool usage is healthy, Ash will call `openai_web_search`, a configured fallback,
`google_places`, and/or `web_fetch` before replying.
...
### Web Search (`openai_web_search`, `exa_search`, `web_search`)

Uses hosted OpenAI search first when the active provider supports it, then Parallel
and optional Exa as configured fallbacks. DeepAgents receives the same allowed
discovery tools; non-hosted results cross Ash's untrusted-output boundary.
...
### Local Search (`google_places`)

Uses Google Places for named businesses, exact branches, phone numbers, Maps URLs,
and current opening hours.
...
# Optional non-hosted search fallbacks
[parallel_search]
enabled = true
api_key = "..."
...
# Authoritative local-business and opening-hours lookup
[google_places]
api_key = "..."
...
```bash
# Confirm optional provider keys and enabled flags
...
Hosted OpenAI search requires an OpenAI model/provider. For fallback providers, add
a valid `[exa_search].api_key` or `[parallel_search].api_key`. For local businesses,
configure `[google_places].api_key`.
