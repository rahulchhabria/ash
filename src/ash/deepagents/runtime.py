...
class DeepAgentsRunner:
    ...
    def create(self) -> Any:
        ...
        # (e.g. "openai:gpt-5.1") to infer the provider.  Bare model IDs like
        # "pioneer/auto" won't work.  If no provider prefix is detected, wrap
        # with "openai:" (Ash uses Pioneer's OpenAI-compatible endpoint).
    ...
    def _normalize_model(model: str) -> str:
        ...
        if ":" in model:
            # Already prefixed, e.g. "openai:gpt-5.1" — pass through
            return model
        # Bare model ID — wrap with "openai:" since Ash routes through
        # Pioneer's OpenAI-compatible endpoint.
