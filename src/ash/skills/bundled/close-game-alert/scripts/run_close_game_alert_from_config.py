#!/usr/bin/env python3
"""Launch the close-game-alert daemon with Telegram config from ~/.ash/config.toml."""
...
def main() -> int:
    ...
    close_game = config.get("skills", {}).get("close-game-alert", {})

    token = (
        os.environ.get("TELEGRAM_BOT_TOKEN")
        or close_game.get("TELEGRAM_BOT_TOKEN")
        or config.get("env", {}).get("TELEGRAM_BOT_TOKEN")
    ...
    chat_id = (
        ...
        or config.get("env", {}).get("TELEGRAM_CHAT_ID")
        or config.get("env", {}).get("telegram_chat_id")
